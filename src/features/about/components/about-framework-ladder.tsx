    ...
    <section id={id} className={cn("w-full scroll-mt-40", className)}>
      <div className="container">
        <div className="mx-auto w-full">
          ...
          <Typography
            variant="h2"
            className="mb-8 border-none text-center text-3xl font-semibold tracking-tight text-balance text-white sm:text-4xl"
          >
            {title}
          </Typography>
            ...
            {levels.map((level, idx) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "group relative flex cursor-pointer items-center gap-2 rounded-full px-4 py-2.5 text-sm font-semibold transition-all duration-200 outline-none select-none",
                    isActive
          ...
          >
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-100 pb-5">
              ...
              <div className="flex items-center gap-2">
                <span className="rounded-full bg-zinc-100 px-3.5 py-1 text-xs font-semibold text-zinc-600">
                  CEFR {activeLevel?.cefr}
                </span>
                {activeIndex >= vstepRange.from &&
                  activeIndex <= vstepRange.to && (
                    <span className="rounded-full bg-red-50 px-3.5 py-1 text-xs font-semibold text-red-600 ring-1 ring-red-600/20 ring-inset">
                      {vstepRange.label}
                    </span>
                  )}
              </div>
